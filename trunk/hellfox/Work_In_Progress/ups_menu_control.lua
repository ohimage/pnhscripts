...
function UpsClnStr( player,command,args )
    ...
    for _, ent in ipairs( ents.GetAll() ) do
        ...
        elseif(ent and ent:IsValid() and (ent:GetClass() or ent:GetModel()) and strArgs and not ent:IsWorld()) then
        
            if not table.HasValue( ignoreList, ent:GetClass() ) then
                print( ent:GetClass() )
                print( string.Trim(strArgs,"*") )
                print( ent:GetClass() )
                print( ent:GetModel() )
                entClassS = string.find(ent:GetClass(), string.Trim(strArgs,"*"))
                entModelS = string.find(ent:GetModel(), string.Trim(strArgs,"*"))
                if not( entClassS == nil or entModelS == nil ) then
                    if ( entClassS <= cs or entModelS <= cs ) then
