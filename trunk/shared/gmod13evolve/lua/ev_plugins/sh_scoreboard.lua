...
function PLUGIN:DrawInfoBar()
	...
	// Content
	local x = self.X + 24
	draw.SimpleText( "Currently playing ", "DermaDefault", x, self.Y + 117, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
	x = x + self:QuickTextSize( "DermaDefault", "Currently playing " )
	draw.SimpleText( GAMEMODE.Name, "DermaDefaultBold", x, self.Y + 117, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
	x = x + self:QuickTextSize( "DermaDefaultBold", GAMEMODE.Name )
	draw.SimpleText( " on the map ", "DermaDefault", x, self.Y + 117, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
	x = x + self:QuickTextSize( "DermaDefault", " on the map " )
	draw.SimpleText( game.GetMap(), "DermaDefaultBold", x, self.Y + 117, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
	x = x + self:QuickTextSize( "DermaDefaultBold", game.GetMap() )
	draw.SimpleText( ", with ", "DermaDefault", x, self.Y + 117, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
	x = x + self:QuickTextSize( "DermaDefault", ", with " )
	draw.SimpleText( #player.GetAll(), "DermaDefaultBold", x, self.Y + 117, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
	x = x + self:QuickTextSize( "DermaDefaultBold", #player.GetAll() )
	local s = ""
	if ( #player.GetAll() > 1 ) then s = "s" end
	draw.SimpleText( " player" .. s .. ".", "DermaDefault", x, self.Y + 117, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
end
...
function PLUGIN:DrawUsergroup( playerinfo, usergroup, title, icon, y )
	...
	draw.SimpleText( title, "DermaDefaultBold", self.X + 40, y + 4, Color( 39, 39, 39, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
