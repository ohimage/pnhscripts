...
function PLUGIN:Call( ply, args )
	if ( ply:EV_HasPrivilege( "Map changing" ) ) then
		if ( args[1] and file.Exists( "maps/" .. args[1] .. ".bsp", "GAME" ) and file.Exists( "gamemodes/" .. ( args[2] or GAMEMODE.FolderName ), LUA_PATH ) ) then
			if ( args[2] ) then
			...
			timer.Simple( 0.5, function() RunConsoleCommand( "gamemode", args[2] or GAMEMODE.FolderName) RunConsoleCommand( "changelevel", args[1] )  end )
		else
