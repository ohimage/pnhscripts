...
function GM:GetTeamScoreInfo()
	...
	for id,pl in pairs( player.GetAll() ) do
		...
		if pl:IsValid() then
			...
			local _ping = pl:Ping()
			local _title = pl:Title()
			local _class = "---"
			if (pl:GetPlayerClass() ~= 0) then
				if (pl:Team() == TEAM_UNDEAD) then
					if (LocalPlayer():Team() == TEAM_UNDEAD or PlayerIsAdmin) then
						_class = UndeadClass[pl:GetPlayerClass()].Name
					else
						_class = "???"
					end
				elseif (pl:Team() == TEAM_HUMAN) then
					if (LocalPlayer():Team() == TEAM_HUMAN or PlayerIsAdmin) then
						_class = HumanClass[pl:GetPlayerClass()].Name
					else
						_class = "???"
					end
				end
			...
			PlayerInfo.Class = _class
			PlayerInfo.Title = _title
...
function GM:HUDDrawScoreBoard()
	...
	-- Draw hostname
	if ( string.len(hostname) > 22 ) then
		surface.SetFont( "DoomMedium" )
	else
		surface.SetFont( "DoomLarge" )
	end
	...
	-- Draw gamemode name (plus author)
	surface.SetTextColor( COLOR_WHITE )
	surface.SetFont( "DoomSmall" )
	local txWidth, txHeight = surface.GetTextSize( gamemodeName )
	surface.SetTextPos(xOffset + (boardWidth / 2) - (txWidth/2), y)
	surface.DrawText( gamemodeName )
	
	-- Some positioning variables (factors of colWidth)
	local TitleX = 7.5
	local TitleW = 2.5
	local ClassX = 5
	-- Next column starts at 3
	...
	surface.SetDrawColor( 255, 255, 255, 20 )
	surface.DrawRect( xOffset + boardWidth - (colWidth*TitleX), y, colWidth*TitleW, boardHeight-y+yOffset )
	...
	surface.SetTextPos( xOffset + boardWidth - (colWidth*TitleX) + 8,	y)	surface.DrawText("Title")
	surface.SetTextPos( xOffset + boardWidth - (colWidth*ClassX) + 8,	y)	surface.DrawText("Class")
	...
	for team,info in pairs(ScoreboardInfo) do
		...
		local teamText = info.TeamName .. "  - Active units: " .. #info.Players
		...
		for index,plinfo in pairs(info.Players) do
			...
			px = xOffset + boardWidth - (colWidth*TitleX) + 8			
			draw.SimpleText( plinfo.Title, "ScoreboardText", px+1, py+1, shadowcolor )
			draw.SimpleText( plinfo.Title, "ScoreboardText", px, py, textcolor )

			px = xOffset + boardWidth - (colWidth*ClassX) + 8			
			draw.SimpleText( plinfo.Class, "ScoreboardText", px+1, py+1, shadowcolor )
			draw.SimpleText( plinfo.Class, "ScoreboardText", px, py, textcolor )
