	...
	if(ValidEntity( ent ) )then
		if(ent:GetClass() == "player" and attacker:GetClass() == "prop_physics"
			...
			if(attacker.PK_NotifySent == nil)then
				if(ValidEntity( attacker ) and ValidEntity( ent ))then
					local ply = attacker.lastTouchedBy
					if(ply != nil)then
						LPRP:Notify( "", ply,Color(255,255,0),"("..ply:SteamID()..")",Color(0,255,255)," tried to prop kill ", ent)
