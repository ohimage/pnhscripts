...
if ( SERVER ) then
			...
			if ( found ) then
				...
				if ( prefix == "sh" or prefix == "cl" ) then
					net.Start( "EV_PluginFile" )
						local EV_PluginsTable = { Title = title, Contents = file.Read( "ev_plugins/" .. plugin, LUA_PATH ) }
						net.WriteTable( EV_PluginsTable )
	...
	net.Receive( "EV_PluginFile", function( len )
		...
		for k, plugin in ipairs( evolve.plugins ) do
			if ( string.lower( plugin.Title ) == string.lower( plugins.Title ) ) then
				found = k
				table.remove( evolve.plugins, k )
			end
		end
		
		RunString( plugin.Contents )
	end)
end
